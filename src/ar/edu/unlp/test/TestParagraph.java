...
public class TestParagraph {
	...
	private AbbreviationDic paragraph;
	...
	private String abbrevTestText = "Veg. Physiol. NADA Venet. test word Astronaut.";
	
	@Before
	public void initDic(){
		try {
			this.paragraph = new AbbreviationDic();
		} catch (IOException e) {
			e.printStackTrace();
			fail(e.getMessage());
		}
		 System.out.println("==> Dic loadded. Size: "+paragraph.getDictionary().size());
	}
	
	@Test
	public void testParagraph() {
		
		Set<String> abb = paragraph.getDictionary();
		 if(abb.size() < 600){
			 fail("abbreviation dictionary is too small");
		 }
		 if(!abb.contains("Zool.")){
			 fail("abbreviation dictionary is not completed");
		 }
		 System.out.println("Dic is ok");
	}
