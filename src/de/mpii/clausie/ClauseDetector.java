...
class ClauseDetector {
    ...
    /**TODO: JMR This is one important method in oder to translate this in future
     * */
    ...
    private static void addNsubjClause(ClausIE clausIE, List<IndexedWord> roots,
        ...
        for (int i = 0; i < ccs.size(); i++) {
            ...
            if (rcmod != null)
                poss = DpUtils.findDescendantRelativeRelation(semanticGraph, root, EnglishGrammaticalRelations.POSSESSION_MODIFIER);
...
//------------------------Set verb or complement, and subject.-------------------------------------------------
            ...
            if (cop != null) {
                ...
                if (!partmod) {
                    ...
                    clause.constituents.add(new TextConstituent("be " + clauseRoot.word(),
                            Constituent.Type.VERB));  //TODO: JMR check in the future for chage to spanish, eliminate be 
                ...
                if (!partmod) {
                	...
                	//JMR: En castellano se elimina el verbo "be" como auxiliar
                    constRoot = new TextConstituent(clauseRoot.word(),Constituent.Type.VERB);
                }
            ...
            if (subject.tag().charAt(0) == 'W' && rcmod != null) { /*JMR: quiere decir si la palabra fue taggeada como una de estas: WDT Wh­determiner,WP Wh­pronoun,WP$ Possessive wh­pronoun o WRB Wh­adverb*/
                ...
                clause.constituents.add(createPossConstituent(semanticGraph, poss, rcmod, subject,
                        Type.SUBJECT));
                rcmod = null;
            } else if (partmod && subject.tag().charAt(0) == 'V') { /*JMR: si es algun verbo*/
